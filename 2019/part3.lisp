...
(defun split-on-comma (str)
  (uiop/utility:split-string str :separator ","))
(defun read-wire-path (path-string)
  (-<> path-string
       (uiop/utility:split-string <> :separator '(#\linefeed))
       (mapcar #'split-on-comma <>)))

;; test
(equalp (read-wire-path (format nil "R75,D30,R83,U83,L12,D49,R71,U7,L72~@
                                     U62,R66,U55,R34,D71,R55,D58,R83"))
        '(("R75" "D30" "R83" "U83" "L12" "D49" "R71" "U7" "L72")
          ("U62" "R66" "U55" "R34" "D71" "R55" "D58" "R83")))

(defun step-direction (direction origin)
  "Returns a point that is one step in the given direction from `origin`"
  (cond ((eql direction #\R) (cons (1+ (first origin)) (rest origin)))
        ((eql direction #\L) (cons (1- (first origin)) (rest origin)))
        ((eql direction #\U) (cons (first origin)      (1+ (rest origin))))
        ((eql direction #\D) (cons (first origin)      (1- (rest origin))))
        (t (error (format nil "invalid direction: [~a]" direction)))))

(defun m-dist (pt-a pt-b)
  (+
   (abs (- (first pt-a) (first pt-b)))
   (abs (- (rest pt-a)  (rest pt-b)))))

(defun find-collisions (line-a line-b)
  (intersection line-a line-b :test #'equalp))

(defun coords-for-segment (segment-string origin)
  "Given a segment descriptor and origin point, returns a list of points representing
   that line segement"
  (let ((direction (aref segment-string 0))
        (steps     (parse-integer segment-string :start 1))
        (result    '()))
    (dotimes (step steps)
      (push (step-direction direction (or (first result) origin)) result))
    result))

(defun path-to-coords (path)
  "Takes a list of segment descriptors (e.g. '(\"R42\" \"U48\") and returns the path
   as a list of coordinates."
  (reduce (lambda (path-so-far next-segment)
            (append (coords-for-segment next-segment (or (first path-so-far) '(0 . 0))) path-so-far))
          path
          :initial-value '()))
  
(defun find-crossings (paths)
  (let ((paths-segments (read-wire-path paths)))
    (find-collisions
     (path-to-coords (first paths-segments))
     (path-to-coords (second paths-segments)))))

;; quick test, they're not guaranteed to be in this order I think but eh
(equalp (find-crossings (format nil "R8,U5,L5,D3~&U7,R6,D4,L4")) '((6 . 5) (3 . 3)))

(defun find-closest-crossing (paths)
  (-<> paths
       (find-crossings <>)
       (mapcar (lambda (point) (cons (m-dist '(0 . 0) point) point)) <>)
       (sort <> #'< :key #'first)
       (first <>)))
       
(defvar *aoc3tst1* "R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83")
(= (first (find-closest-crossing *aoc3tst1*)) 159)

(defvar *aoc3tst2* "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
(= (first (find-closest-crossing *aoc3tst2*)) 135)

(defvar *aoc3input* "R1009,U286,L371,U985,R372,D887,R311,U609,L180,D986,L901,D592,R298,U955,R681,D68,R453,U654,L898,U498,R365,D863,L974,U333,L267,D230,R706,D67,L814,D280,R931,D539,R217,U384,L314,D162,L280,U484,L915,D512,L974,D220,R292,U465,L976,U837,R28,U68,L98,D177,L780,U732,R696,D412,L715,U993,L617,U999,R304,D277,R889,D604,R199,U498,R302,U958,R443,U957,R453,U362,R704,U301,R813,U404,L150,D673,L407,D233,L901,D965,R602,U615,R496,U467,R849,U530,L205,D43,R709,U127,L35,U801,L565,D890,R90,D763,R95,D542,R84,D421,L298,D58,R794,U722,R205,U830,L149,D759,L950,D708,L727,U401,L187,D598,L390,D469,R375,U985,L723,U63,L983,D39,L160,U276,R822,D504,L298,D484,L425,U228,L984,D623,L936,U624,L851,D748,L266,D576,L898,U783,L374,D276,R757,U89,L649,U73,L447,D11,L539,U291,L507,U208,R167,D874,L596,D235,R334,U328,R41,D212,L544,D72,L972,D790,L282,U662,R452,U892,L830,D86,L252,U701,L215,U179,L480,U963,L897,U489,R223,U757,R804,U373,R844,D518,R145,U304,L24,D988,R605,D644,R415,U34,L889,D827,R854,U836,R837,D334,L664,D883,L900,U448,R152,U473,R243,D147,L711,U642,R757,U272,R192,U741,L522,U785,L872,D128,L161,D347,L967,D295,R831,U535,R329,D752,R720,D806,R897,D320,R391,D737,L719,U652,L54,D271,L855,D112,R382,U959,R909,D687,L699,U892,L96,D537,L365,D182,R886,U566,R929,U532,L255,U823,R833,U542,R234,D339,R409,U100,L466,U572,L162,U843,L635,D153,L704,D317,L534,U205,R611,D672,L462,D506,L243,U509,L819,D787,R448,D353,R162,U108,R850,D919,R259,U877,R50,D733,L875,U106,L890,D275,L904,U849,L855,U314,L291,U170,L627,U608,R783,U404,R294
...
;; doing it this way may not be the wisest choice as
;; this takes 4 minutes to run on the real inputs lol


;; part 2
(defun intersection-cost (path-a path-b point)
  "Given two paths and an intersection, finds the cost for the point as described in day 3 part 2"
  (+ (- (length path-a) (position point path-a :test 'equalp :from-end t))
     (- (length path-b) (position point path-b :test 'equalp :from-end t))))

(defun get-cheapest-intersection (path-a path-b intersections)
  (-<> intersections
       (mapcar (lambda (pt) (cons (intersection-cost path-a path-b pt) pt)) <>)
       (sort <> #'< :key #'first)))

(defun find-cheapest-crossing (path-a path-b)
  (let ((intersections (find-collisions path-a path-b)))
    (get-cheapest-intersection path-a path-b intersections)))

(defun strpath->cheapest-crossings (paths)
  (let* ((paths-segments (read-wire-path paths))
         (path-a-segs (first paths-segments))
         (path-b-segs (second paths-segments))
         (path-a-pts  (path-to-coords path-a-segs))
         (path-b-pts  (path-to-coords path-b-segs)))
    (find-cheapest-crossing path-a-pts path-b-pts)))
